...
async def get_case_core(conn, case_id: str) -> dict:
    """Pull case metadata + paragraphs from legal_cases and legal_paragraphs."""
    ...
    # Fetch citations if table exists
    citations = []
    try:
...
async def _stream_ollama_json(prompt: str, max_tokens: int = 1500):
...
async def get_arguments_cached(case_id: str):
    ...
    return {
        "case_id":      case_id,
        "arguments":    safe_parse_jsonb(row["arguments_json"]),
...
async def generate_arguments(case_id: str, force_regenerate: bool = False):
    ...
    try:
        ...
        if not force_regenerate:
            ...
            if cached:
                return JSONResponse({'arguments': safe_parse_jsonb(cached['arguments_json']), 'cached': True})
...
- key_legal_test: e.g. "Three-part proportionality test" or null"""
    ...
    # ── Call Ollama and cache on success ───────────────────────────────────────
    data = await _call_ollama_json(prompt, max_tokens=2000)

    # Save to DB cache if no errors
    ...
    return JSONResponse({'arguments': data, 'cached': False})
...
async def generate_quick_summary(case_id: str):
    ...
    try:
        ...
        if cached:
            async def serve_cached():
                brief_data = safe_parse_jsonb(cached['brief_json'])
                yield f"data: {json.dumps({**brief_data, 'cached': True, 'done': True})}\n\n"
    ...
    # ── DB-only fast path — if judgment exists, skip LLM ────────────
    judgment_text = (case.get("judgment") or "").strip()

    if judgment_text and len(judgment_text) > 40:
        # We have enough in the DB to build a good summary without LLM
        one_liner_db   = f"Held: {judgment_text[:120].rstrip('.,')}." 
        summary_30s_db = judgment_text[:350]
...
Judgment excerpt: {judgment_text[:500]}
...
The one_liner MUST start with 'Held:'. Be specific — name the right, the test, or the principle."""
    ...
    try:
        paras = await conn4.fetch("""
            SELECT para_no, text, para_type FROM legal_paragraphs
            WHERE case_id = $1
            ORDER BY para_no LIMIT 8
        """, case_id)
        para_text = " ".join(p["text"][:300] for p in paras)
    finally:
        await conn4.close()
...
Identify {max_i} issues maximum. Only Indian law. Only actionable issues."""
    ...
    async def stream_with_case_search():
        # Stream while accumulating LLM response
        issues_data = await _stream_ollama_json(prompt, max_tokens=1200)

        if issues_data.get("parse_error"):
            yield f"data: {json.dumps({'error': 'Failed to identify legal issues', 'done': True})}\n\n"
            return

        # DB search for real cases matching each issue
        conn = await get_conn()
        try:
            for issue_obj in issues_data.get("issues", []):
                ...
                if acts:
                    act_terms = [a.split(" ")[0] for a in acts[:2]]
                    act_nums  = []
                    for a in acts[:2]:
                        parts = a.split()
                        if len(parts) >= 2:
                            act_nums.append(parts[-1])

                    if act_terms and act_nums:
                        try:
                            rows = await conn.fetch("""
                                SELECT case_id, case_name, court, year, outcome
                                FROM legal_cases
                                WHERE acts_referred && $1
                                ORDER BY year DESC
                                LIMIT 4
                ...
                if not relevant_cases and search_q:
                    ...
                    if words:
                        like_clauses = " OR ".join(
                            f"(case_name ILIKE ${i+1} OR judgment ILIKE ${i+1})"
    ...
    return StreamingResponse(stream_with_case_search(), media_type="text/event-stream")
