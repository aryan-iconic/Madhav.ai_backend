...
async def fetch_case_core(conn, case_id: str) -> dict:
    case = await conn.fetchrow("""
        SELECT case_id, case_name, citation, court, date_of_judgment,
               outcome, ratio_decidendi, acts_sections, judges, bench_strength
...
async def fetch_cached_arguments(conn, case_id: str) -> Optional[dict]:
    ...
    try:
        ...
        return dict(row["arguments_json"]) if row else None
    except Exception:
        return None
...
def _compact_case_text(ctx: dict, max_paras: int = 35) -> str:
    c = ctx["case"]
    paras = "\n".join(
        f"Para {p['para_number']} [{p['para_type'] or 'general'}]: {p['para_text'][:350]}"
        for p in ctx["paras"][:max_paras]
    )
    return (
        f"Case: {c['case_name']} ({c.get('citation','N/A')})\n"
        f"Court: {c['court']} | Outcome: {c.get('outcome','N/A')}\n"
        f"Ratio (DB): {(c.get('ratio_decidendi') or 'N/A')[:300]}\n\n"
        f"PARAGRAPHS:\n{paras}"
    )
...
async def counter_arguments(case_id: str, force_regenerate: bool = False):
    ...
    try:
        ...
        if not force_regenerate:
            ...
            if cached:
                async def serve():
                    yield f"data: {json.dumps({**cached, 'cached': True, 'done': True})}\n\n"
                return StreamingResponse(serve(), media_type="text/event-stream")
    ...
    async def stream_and_cache():
        data = await _call_ollama_json(prompt, max_tokens=1800)
        ...
        yield f"data: {json.dumps({**data, 'cached': False, 'done': True})}\n\n"
...
async def fact_law_separation(case_id: str, force_regenerate: bool = False):
    ...
    async def stream_and_cache():
        data = await _call_ollama_json(prompt, max_tokens=2500)
...
async def reasoning_full(case_id: str):
    ...
    try:
        ...
        reasoning_map = {r["cache_key"]: dict(r["data_json"]) for r in reasoning_rows}

    finally:
        await conn.close()

    arguments        = dict(args_row["arguments_json"])    if args_row    else None
    quick_summary    = dict(summary_row["brief_json"])     if summary_row else None
